...
(`VPCId`)|`**__Blank string__**`|Required for existing VPC target.|Existing subnet ID
(`VPCSubnetId`)|`**__Blank string__**`|Required for an existing VPC target. Subnet ID in the existing VPC in which EMR Cluster and SageMaker Notebook Instances will be launched. A private subnet is recommended.|Existing subnet type
...
(`KMSEbsKeyArn`)|`**__Blank string__**`|(Optional) The full KMS Key ARN if Region level EBS encryption is enabled. This does not automatically encrypt your AMI.
|===
...
(`TagOwner`)|`**__Blank string__**`|(Optional) Owner for default resource tagging. Suggested values are <User Name>, <Department Name>, <Project Name>, etc.
|===
